...
class FixedParams(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.register_parameter("dyna_params", torch.nn.Parameter(torch.tensor([0., 0., 0., 0., 0., 0.])))
...
if __name__ == '__main__':
    ...
    for t in tqdm(range(epochs)):
        ...
        training_segment_dataset = model.prepare_dataset(training_dataset, epoch=t, writer=writer, plot=True,
                                                         type=prepare_typ, set_params=True, cal=cal, beta=beta)
        ...
        for index_batch in tqdm(loader):
            ...
            loss = model.calculate_loss(training_segment_dataset[index_batch], training_dataset, type=loss_typ, epoch=t,
                                        set_params=True, cal=cal, beta=beta)
        ...
        test_segment_dataset = model.prepare_dataset(test_dataset, type=prepare_typ, epoch=t, set_params=True, cal=cal,
                                                     beta=beta)
