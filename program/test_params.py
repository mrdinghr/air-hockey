...
def plot_trajectory(params, trajectories, epoch=0, writer=None):
    ...
    for trajectory_index, data_set in enumerate(trajectories):
        ...
        # plt.figure()
        plt.scatter(data_set[:, 0], data_set[:, 1], label='record data', c='g')
        ...
        plt.scatter(data_set[:, 3] - data_set[0, 3], data_set[:, 0], label='record data', c='g')
        plt.scatter(time_list, state_list[:, 0], label='predicted data', c='b')
        plt.legend()
        plt.subplot(3, 1, 2)
        plt.title('y position')
        plt.scatter(data_set[:, 3] - data_set[0, 3], data_set[:, 1], label='record data', c='g')
        plt.scatter(time_list, state_list[:, 1], label='predicted data', c='b')
        plt.legend()
        plt.subplot(3, 1, 3)
        plt.title('theta')
        plt.scatter(data_set[:, 3] - data_set[0, 3], data_set[:, 2], label='record data', c='g')
        plt.scatter(time_list, state_list[:, 4], label='predicted data', c='b')
        plt.legend()
...
def EKF_plot_with_state_list(EKF_state_list, trajectory, writer=None, epoch=0, trajectory_index=None):
    ...
    plt.subplot(3, 1, 1)
    plt.title('x position')
    plt.scatter(trajectory[0:, 3] - trajectory[0, 3], EKF_state_list[:, 0], label='EKF trajectory', c='b')
    plt.scatter(trajectory[:, 3] - trajectory[0, 3], trajectory[:, 0], label='recorded trajectory', c='g')
    plt.legend()
    # position y
    plt.subplot(3, 1, 2)
    plt.title('y position')
    plt.scatter(trajectory[0:, 3] - trajectory[0, 3], EKF_state_list[:, 1], label='EKF trajectory', c='b')
    plt.scatter(trajectory[:, 3] - trajectory[0, 3], trajectory[:, 1], label='recorded trajectory', c='g')
    plt.legend()
    plt.subplot(3, 1, 3)
    plt.title('theta')
    plt.scatter(trajectory[0:, 3] - trajectory[0, 3], EKF_state_list[:, 4], label='EKF trajectory', c='b')
    plt.scatter(trajectory[:, 3] - trajectory[0, 3], trajectory[:, 2], label='recorded trajectory', c='g')
    plt.legend()
    if writer != None:
        writer.add_figure('trajectory_'+str(trajectory_index)+'/position', plt.gcf(), epoch)
    ...
    plt.subplot(3, 1, 1)
    plt.title('x velocity')
    plt.scatter(trajectory[0:, 3] - trajectory[0, 3], EKF_state_list[:, 2], label='EKF trajectory', c='b')
    plt.scatter(trajectory[:-1, 3] - trajectory[0, 3], x_velocity, label='recorded trajectory', c='g')
    plt.legend()
    plt.subplot(3, 1, 2)
    plt.title('y velocity')
    plt.scatter(trajectory[0:, 3] - trajectory[0, 3], EKF_state_list[:, 3], label='EKF trajectory', c='b')
    plt.scatter(trajectory[:-1, 3] - trajectory[0, 3], y_velocity, label='recorded trajectory', c='g')
    plt.legend()
    plt.subplot(3, 1, 3)
    plt.title('rotate velocity')
    plt.scatter(trajectory[0:, 3] - trajectory[0, 3], EKF_state_list[:, 5], label='EKF trajectory', c='b')
    plt.scatter(trajectory[:-1, 3] - trajectory[0, 3], theta_velocity, label='recorded trajectory', c='g')
    plt.legend()
    if writer != None:
        writer.add_figure('trajectory_'+str(trajectory_index)+'/velocity', plt.gcf(), epoch)
